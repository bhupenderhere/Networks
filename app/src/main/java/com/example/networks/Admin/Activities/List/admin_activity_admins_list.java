package com.example.networks.Admin.Activities.List;
...
import com.example.networks.Admin.Activities.Adapter.AdminAdapter;
...
public class admin_activity_admins_list extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.admin_activity_admins_list);

        // Style The Action Bar
        Utility.activityActionBar("Admins List", Objects.requireNonNull(getSupportActionBar()), this);
        ...
        RecyclerView recyclerView = findViewById(R.id.adminsList);
        ...
        if (Utility.isNetworkStatusAvailable(this)) {
            ...
            FirebaseFirestore.getInstance().collection("admin").orderBy("id").get().addOnCompleteListener(task -> {
                if (task.isSuccessful()) {
                    QuerySnapshot querySnapshot = task.getResult();
                    if (querySnapshot != null && querySnapshot.size() > 0) {
                        ArrayList<AdminData> data = new ArrayList<>();
                        for (DocumentSnapshot document : querySnapshot.getDocuments()) {
                            data.add(document.toObject(AdminData.class));
                        }
                        AdminAdapter adapter = new AdminAdapter(this, data);
                        recyclerView.setAdapter(adapter);
                        progressBar.setVisibility(View.GONE);
                    } else {
                        progressBar.setVisibility(View.GONE);
                        MaterialAlertDialogBuilder builder = new MaterialAlertDialogBuilder(admin_activity_admins_list.this, R.style.AlertDialogTheme);
                    ...
                    Utility.makeToast("Something Went Wrong!", getApplicationContext());
            });
        }
    }
}