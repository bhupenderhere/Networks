...
import com.example.networks.Admin.Activities.Adapter.UserAdapter;
...
public class admin_activity_users_list extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Utility.activityActionBar("Users List", Objects.requireNonNull(getSupportActionBar()), this);
        ...
        if (Utility.isNetworkStatusAvailable(this)) {
            ...
            FirebaseFirestore.getInstance().collection("users").orderBy("department").get().addOnCompleteListener(task -> {
                if (task.isSuccessful()) {
                    QuerySnapshot querySnapshot = task.getResult();
                    if (querySnapshot != null && querySnapshot.size() > 0) {
                        ArrayList<UsersData> data = new ArrayList<>();
                        for (DocumentSnapshot document : querySnapshot.getDocuments()) {
                            data.add(document.toObject(UsersData.class));
                        }
                        ...
                        progressBar.setVisibility(View.GONE);
                        MaterialAlertDialogBuilder builder = new MaterialAlertDialogBuilder(admin_activity_users_list.this, R.style.AlertDialogTheme);
